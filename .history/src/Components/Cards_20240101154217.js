...
const StyledCardsContainer = styled(Container)`
  ...
  @media (max-width: 320px) {
    margin-right: auto;
    margin-left: auto;
    padding-right: 2rem; /* Измените значение отступа справа по вашему усмотрению */
    padding-left: 2rem; /* Измените значение отступа слева по вашему усмотрению */
    align-items: center;
  }
...
const Cards = () => {
  ...
  return (
    <StyledCardsContainer style={{ alignItems: 'center' }}>
      <Row xs={1} md={1} lg={3} xl={4}>
        {Object.keys(textData).map((key, index) => (
          <StyledCol key={key} style={{ paddingBottom: '2rem', margin: '3.5rem' }}>
