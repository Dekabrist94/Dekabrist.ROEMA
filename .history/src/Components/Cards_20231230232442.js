...
const StyledCardsContainer = styled(Container)`
  ...
  /* Медиа-запросы для адаптации под разные ширины экранов */
  ...
  /* Добавьте дополнительные медиа-запросы при необходимости */
`;
      ...
      <Row>
        {Object.keys(textData).map((key) => (
          <Col key={key} md={6} lg={4} xl={3} className="mb-4">
            {/* Используем классы Bootstrap для определения количества колонок на разных экранах */}
            <Card>
              <Card.Img
                variant="top"
